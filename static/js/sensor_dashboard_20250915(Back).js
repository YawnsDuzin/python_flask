...
async function loadLocalDatabase() {
  try {
    ...
    const response = await fetch(`${SERVER_URL}/api/sensor-dashboard/init-data?api_key=${API_KEY}`);
    if (!response.ok) {
  ...
  activeDevices.forEach((device, index) => {
    ...
    try {
      ...
      deviceEventSource.onerror = (error) => {
        ...
        if (deviceEventSource.readyState !== 1) {  // OPEN이 아닌 모든 상태에서 재연결
          try {
          ...
          if (reconnectAttempts[deviceKey] <= RECONNECT_CONFIG.maxAttempts) {
            // 지수 백오프 계산
            delay = Math.min(
              RECONNECT_CONFIG.initialDelay * Math.pow(
          ...
          if (reconnectAttempts[deviceKey] <= RECONNECT_CONFIG.maxAttempts) {
            addToLog(
              `[재연결] ${monitorName} ${reconnectAttempts[deviceKey]}/${RECONNECT_CONFIG.maxAttempts}회 시도 (${(delay/1000).toFixed(1)}초 후)`,
              "warning"
            );
          } else {
            addToLog(
...
function reconnectToDevice(device, index) {
  ...
  try {
    ...
    const newEventSource = new EventSource(
      `${deviceUrl}/api/public-sensor-stream?api_key=${encodeURIComponent(API_KEY)}`
    );
    ...
    newEventSource.onerror = (error) => {
      ...
      if (newEventSource.readyState !== 1) {
        ...
        if (reconnectAttempts[deviceKey] <= RECONNECT_CONFIG.maxAttempts) {
          addToLog(
            `[재연결] ${monitorName} ${reconnectAttempts[deviceKey]}/${RECONNECT_CONFIG.maxAttempts}회 재시도 (${(delay/1000).toFixed(1)}초 후)`,
            "warning"
          );
        } else {
          addToLog(
    ...
    addToLog(
      `[재연결 실패] ${monitorName}: ${error.message}`,
      "danger"
    );
...
function convertSensorToDynamicLayout(sensorElement) {
  ...
  const currentStatusClass = statusElement ? statusElement.className : 'connection-status inactive';
  const currentStatusTitle = statusElement ? statusElement.title : '연결 상태: 대기중';
...
function setFont(fontOpt) {
  ...
  return `font-family: ${fontName};font-weight: ${
    fontOpt.fontWeight
  };font-size: ${fontOpt.fontSize}px;color: ${setColor(
    fontOpt.fontColor
    ...
    eventSource.forEach((es, index) => {
      if (es) {
        ...
        if (device) {
          ...
          if (es.readyState !== 1) {  // OPEN이 아닌 경우
            console.log(`[모니터] ${monitorName} 연결 상태 이상 감지 (readyState: ${es.readyState})`);
            
            // 재연결 타이머가 없는 경우에만 재연결 시도
            if (!reconnectTimers[deviceKey]) {
              addToLog(`[모니터] ${monitorName} 재연결 필요 - 재연결 시작`, "warning");
...
function setGridColumns(el, len) {
  ...
  if (len <= 4) {
    ...
    gTemplateRows = `repeat(${rows}, minmax(auto, ${(72 - rows * 0.3) / rows}rem))`;
  }
