...
async function connectSSE() {
  ...
  // 먼저 현재 서버의 SSE에 연결 (브라우저 리프레시 이벤트 수신용)
  try {
    const currentServerUrl = window.location.origin;
    console.log(`[SSE] 현재 서버에 연결 시도: ${currentServerUrl}`);
    console.log(`[SSE] 리프레시 스트림 엔드포인트: ${currentServerUrl}/api/refresh-stream`);

    // 인증이 필요없는 리프레시 전용 엔드포인트 사용
    const currentServerSSE = new EventSource(`${currentServerUrl}/api/refresh-stream`);

    currentServerSSE.onopen = () => {
      console.log(`[SSE] 현재 서버 연결 성공: ${currentServerUrl}`);
      addToLog(`[현재 서버] SSE 연결 성공 - 브라우저 리프레시 수신 준비`, "info");
    };

    currentServerSSE.onmessage = (event) => {
      console.log("[SSE DEBUG] 현재 서버에서 메시지 수신:", event.data);

      try {
        const data = JSON.parse(event.data);
        console.log("[SSE DEBUG] 파싱된 데이터:", data);

        // 이벤트 타입 확인
        if (data.type) {
          console.log("[SSE DEBUG] 이벤트 타입:", data.type);

          // 연결 확인 메시지
          if (data.type === "connected") {
            console.log("[SSE] 리프레시 스트림 연결 확인:", data.message);
            return;
          }

          // 브라우저 리프레시 이벤트 처리
          if (data.type === "browser_refresh") {
            console.log("[REFRESH] 브라우저 리프레시 이벤트 수신:", data);
            console.log("[REFRESH] 타임스탬프:", data.timestamp);
            console.log("[REFRESH] 메시지:", data.message);

            addToLog("[알림] 관리자가 브라우저 리프레시를 요청했습니다. 3초 후 페이지를 새로고침합니다.", "info");
            ...
            const countdownInterval = setInterval(() => {
              countdown--;
              if (countdown > 0) {
                addToLog(`[알림] ${countdown}초 후 새로고침...`, "info");
              } else {
                clearInterval(countdownInterval);
                console.log("[REFRESH] 페이지 새로고침 실행");
        ...
        // JSON 파싱 실패 시 상세 로그
        console.debug("[SSE] 메시지 파싱 오류:", e);
        console.debug("[SSE] 원본 데이터:", event.data.substring(0, 100) + "...");
      }
    };

    currentServerSSE.onerror = (error) => {
      console.error(`[SSE] 현재 서버 연결 오류:`, error);
      console.error(`[SSE] 연결 상태: ${currentServerSSE.readyState}`);

      // readyState: 0=CONNECTING, 1=OPEN, 2=CLOSED
      if (currentServerSSE.readyState === 2) {
        console.error("[SSE] 연결이 완전히 종료됨 (CLOSED)");
        addToLog("[경고] 현재 서버 SSE 연결 실패 - 인증 필요할 수 있습니다", "warning");
      } else if (currentServerSSE.readyState === 0) {
    ...
    // 현재 서버 SSE를 배열에 추가
    eventSource.push(currentServerSSE);

  } catch (error) {
    console.error("[SSE] 현재 서버 연결 실패:", error);
    addToLog("[경고] 현재 서버 SSE 연결 실패", "warning");
  }
  ...
  activeDevices.forEach((device, index) => {
    ...
    try {
      ...
      deviceEventSource.onmessage = (event) => {
        // 디버깅용 로그: 모든 수신 메시지 기록
        console.log(`[SSE DEBUG] ${monitorName} 메시지 수신:`, event.data);
      ...
      // eventSource 배열은 이미 초기화되어 있고, 현재 서버 SSE가 포함되어 있음
...
function reconnectToDevice(device, index) {
  ...
  try {
    ...
    newEventSource.onmessage = (event) => {
      // 디버깅용 로그: 모든 수신 메시지 기록 (재연결 후)
      console.log(`[SSE DEBUG-재연결] ${monitorName} 메시지 수신:`, event.data);
...
function handleMessage(event) {
  try {
    ...
    if (data.type === "browser_refresh") {
      console.log("[REFRESH] 브라우저 리프레시 이벤트 수신:", data);
      addToLog("[알림] 관리자가 브라우저 리프레시를 요청했습니다. 페이지를 새로고침합니다.", "info");

      // 디버깅용 로그
      console.log("[REFRESH] 리프레시 타임스탬프:", data.timestamp);
      console.log("[REFRESH] 리프레시 메시지:", data.message);
      console.log("[REFRESH] 수신 장비:", event.deviceInfo);

      // 1초 후에 페이지 새로고침 (사용자가 알림을 볼 수 있도록)
      setTimeout(() => {
        console.log("[REFRESH] 페이지 새로고침 실행");
        window.location.reload(true); // true: 캐시를 무시하고 서버에서 새로 로드
      }, 1000);
...
function setFont(fontOpt) {
  ...
  return `${
    fontOpt.fontName !== "" && `font-family: ${fontName};`
  }font-weight: ${fontOpt.fontWeight || "Bold"};${
    fontOpt.isSize && `font-size: ${fontOpt.fontSize}px;`
  }color: ${setColor(fontOpt.fontColor)};background-color: ${setColor(
    fontOpt.bgColor
  )};`;
}
  ...
  if (isFixedLayout) {
    ...
    ranges
      ...
      .forEach((range, index) => {
        rangeHtml += `
        <div class="range-item range-${index + 1}">
          <div class="range-item-head" style="${setFont(
            range.title
          )}">${range.title.value
          .split("|")
          .map((str) => str.trim())
          .join("<br />")}</div>
          <div class="range-item-value" style="${setFont(
            range.value
          )}">${range.value.value
          .split("|")
          .map((str) => str.trim())
    ...
    const cardHtml = `
      <div class="sensor-item" id="${cardId}">
        <div class="sensor-top">
          <div class="sensor-top-tit" style="background: ${setColor(
            ...
            <span class="sensor-tit" style="${
              title.isSize && `font-size: ${title.fontSize}px;`
            }color: ${setColor(
      title.fontColor
      ...
      title.value
            ...
            <span class="sensor-format" style="${
              unit.isSize && `font-size: ${unit.fontSize}px;`
            }color: ${setColor(unit.fontColor)};">${unit.value}</span>
      ...
      data.isSize && `font-size: ${data.fontSize}px;`
